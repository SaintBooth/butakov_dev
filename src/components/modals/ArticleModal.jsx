...
export default function ArticleModal({ post, onClose, onContactClick }) {
  return (
    <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 sm:p-6">
      ...
      <div className="relative bg-white/90 backdrop-blur-2xl border border-white rounded-[2rem] w-full max-w-3xl max-h-[90vh] overflow-y-auto no-scrollbar shadow-2xl animate-in fade-in zoom-in duration-200 flex flex-col">
        <button onClick={onClose} className="sticky top-4 right-4 ml-auto -mb-12 w-10 h-10 bg-white/60 backdrop-blur-md border border-white hover:bg-white/80 rounded-full flex items-center justify-center text-slate-600 transition-all z-20 shadow-sm mr-4">
          <X className="w-5 h-5" />
        </button>
        ...
        <div className="p-8 sm:p-12 pt-8 relative bg-white/40">
          ...
          <h2 className="text-3xl sm:text-4xl font-extrabold text-slate-900 mt-6 mb-8 leading-tight">{post.title}</h2>
          <div className="prose prose-slate prose-teal max-w-none">
            <p className="text-lg text-slate-600 font-medium leading-relaxed whitespace-pre-line">{post.content}</p>
          </div>
          <div className="mt-12 pt-8 border-t border-slate-100 flex flex-col items-center text-center">
            <div className="w-16 h-16 rounded-full bg-slate-100 border-2 border-white shadow-md overflow-hidden mb-4">
              <img src="butakov-01.png" alt="Александр Бутаков" className="w-full h-full object-cover p-2 bg-slate-900" />
            </div>

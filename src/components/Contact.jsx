            ...
            <form onSubmit={handleSubmit} className="space-y-6">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div className="space-y-2">
                  ...
                  <input name="name" required type="text" placeholder="Иван Иванов" className="w-full bg-white/50 backdrop-blur-sm border border-white/80 rounded-xl px-5 py-4 text-slate-900 placeholder:text-slate-400 focus:outline-none focus:ring-2 focus:ring-teal-500/50 focus:bg-white transition-all font-medium shadow-sm" />
                </div>
                <div className="space-y-2">
                  <label className="text-sm font-bold text-slate-700">Telegram / Телефон *</label>
                  <input name="contact" required type="text" placeholder="@username или +7..." className="w-full bg-white/50 backdrop-blur-sm border border-white/80 rounded-xl px-5 py-4 text-slate-900 placeholder:text-slate-400 focus:outline-none focus:ring-2 focus:ring-teal-500/50 focus:bg-white transition-all font-medium shadow-sm" />
                </div>
              </div>
              <div className="space-y-2">
                <label className="text-sm font-bold text-slate-700">Что вас интересует?</label>
                <select name="service" value={selectedService} onChange={(e) => onServiceChange(e.target.value)} className="w-full bg-white/50 backdrop-blur-sm border border-white/80 rounded-xl px-5 py-4 text-slate-900 focus:outline-none focus:ring-2 focus:ring-teal-500/50 focus:bg-white transition-all appearance-none cursor-pointer font-medium shadow-sm">
              ...
              <div className="space-y-2">
                ...
                <textarea name="message" rows={4} placeholder="Опишите вашу бизнес-задачу, каких результатов хотите достичь или прикрепите ссылку на ТЗ..." className="w-full bg-white/50 backdrop-blur-sm border border-white/80 rounded-xl px-5 py-4 text-slate-900 placeholder:text-slate-400 focus:outline-none focus:ring-2 focus:ring-teal-500/50 focus:bg-white transition-all resize-none font-medium shadow-sm" />
              </div>
              <button type="submit" disabled={isSubmitting} className="w-full py-5 rounded-xl bg-slate-900 text-white font-bold text-lg hover:bg-teal-500 transition-all shadow-xl shadow-slate-900/20 flex items-center justify-center gap-2 group disabled:opacity-70 disabled:cursor-not-allowed active:scale-[0.98]">
